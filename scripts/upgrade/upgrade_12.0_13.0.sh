...
function check_python_executable() {
    ...
    if which python3 2>/dev/null 1>&2; then
        ...
        PYTHON="python"$(python --version | cut -b 8-20)
...
function set_env_config () {
    if [ -z "${JWT_PRIVATE_KEY}" ]; then
        ...
        export SEAFILE_CCNET_DB_NAME=${SEAFILE_CCNET_DB_NAME:-ccnet_db}
        export SEAFILE_SEAFILE_DB_NAME=${SEAFILE_SEAFILE_DB_NAME:-seafile_db}
        export SEAFILE_SEAHUB_DB_NAME=${SEAFILE_SEAHUB_DB_NAME:-seahub_db}
...
function update_database() {
    ...
    if ! $PYTHON "${db_update_helper}" 13.0.0; then
        echo
